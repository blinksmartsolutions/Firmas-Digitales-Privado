...
class pdfEnviadosGerente(TimestampMixin):
    ...
    nombre_documento = models.TextField(
        max_length=255, verbose_name="Nombre del documento", null=True
    )
...
class solicitudOrdendeCompras(TimestampMixin):
    ...
    N_documento = models.CharField(
        max_length=22, verbose_name="Numero del documento", null=True
    )
    descripcion_documento = models.CharField(
        max_length=460, blank=True, null=True, verbose_name="Descripción del documento"
    )

    usuario = models.ForeignKey(UsuariosTrime, on_delete=models.CASCADE)
    requisicionCancelada = models.ForeignKey(
        pdfEnviadosAlmacen, on_delete=models.CASCADE
    )
...
class cotizaciondeCompras(TimestampMixin):
    ...
    usuario = models.ForeignKey(UsuariosTrime, on_delete=models.CASCADE)
    solcitudOrdendeCompras = models.ForeignKey(
        solicitudOrdendeCompras, on_delete=models.CASCADE
    )

    nombre_documento = models.CharField(max_length=255)
    documento_pdf = models.FileField(
        upload_to=change_name, validators=[validate_file_extension, file_size]
    )
    ESTATUS_DOCUMENTO = models.CharField(
        choices=ESTATUS,
        max_length=22,
...
class pdf2EnviadoGGeneral(TimestampMixin):
    ...
    usuario = models.ForeignKey(UsuariosTrime, on_delete=models.CASCADE)
    cotizacionAcecptada = models.ForeignKey(
        solicitudOrdendeCompras, on_delete=models.CASCADE
    )
    documento_pdf = models.FileField(
        upload_to=change_name,
...
class Signature(TimestampMixin):
    ...
    pdf_file_firmado = models.FileField(
        upload_to="pdfs/", blank=True
