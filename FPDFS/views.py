...
def sign_pdf(request,id_PDF):
    ...
    if user_is_in_group(request.user, 'GERENCIA GENERAL'):
        ...
        context = {'message': 'Contenido exclusivo para administradores',
        ...
        context = {'message': 'Contenido para usuarios básicos',
                   "requisiciones_enviadas":requisiciones_enviadas,
                   "user_id":user_id,
                   }
...
def dashboard(request):
    ...
    if user_is_in_group(request.user, 'GERENCIA GENERAL'):
        ordenes_compra_enviadas = pdf2EnviadoGGeneral.objects.filter(Q(ESTATUS_DOCUMENTO="Rechazado por GG")|Q(ESTATUS_DOCUMENTO="Firmado por GG")|Q(ESTATUS_DOCUMENTO="Rechazado por GG")|Q(ESTATUS_DOCUMENTO='Firmado por Compras')).all()
        ordenes_compra_enviadas_N = ordenes_compra_enviadas.filter(ESTATUS_DOCUMENTO='Firmado por Compras')
        ordenes_compra_enviadas_F = ordenes_compra_enviadas.filter(Q(ESTATUS_DOCUMENTO="Firmado por GG"))
        ordenes_compra_enviadas_R = ordenes_compra_enviadas.filter(Q(ESTATUS_DOCUMENTO="Rechazado por GG"))
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para GERENTE GENERAL')
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para GERENTE DEL DEPARTAMENTO DE ADMINISTRACION')    
 ...
 ## =============================== REQUISICIONES ===============================   
        ...
        requisiciones_enviadas_N = requisiciones_enviadas.filter( ESTATUS_DOCUMENTO='ENVIADO')
        requisiciones_enviadas_F = requisiciones_enviadas.filter(ESTATUS_DOCUMENTO="FIRMADO")
        requisiciones_enviadas_R = requisiciones_enviadas.filter(Q(ESTATUS_DOCUMENTO="Rechazado por Almacen")| Q(ESTATUS_DOCUMENTO="Rechazado por Gerente"))
        requisiciones_enviadas_A = requisiciones_enviadas.filter(ESTATUS_DOCUMENTO="Aceptado por Almacen")
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para usuarios básicos')     
        ...
        requisiciones_enviadas = pdfEnviadosGerente.objects.filter(usuario__Departamento_Usuario=request.user.Departamento_Usuario).all()
        ...
        messages.success(request, f'Hola {request.user}, contenido exclusivo para GERENTE') 
        ...
        requisiciones_enviadas_N = requisiciones_enviadas.filter(documento_pdf__ESTATUS_DOCUMENTO='ENVIADO')
        requisiciones_enviadas_F = requisiciones_enviadas.filter(documento_pdf__ESTATUS_DOCUMENTO="FIRMADO")
        requisiciones_enviadas_R = requisiciones_enviadas.filter(documento_pdf__ESTATUS_DOCUMENTO="Rechazado por Almacen")
        requisiciones_enviadas_A = requisiciones_enviadas.filter(documento_pdf__ESTATUS_DOCUMENTO="Aceptado por Almacen")
        ...
        solicitud_ordenes_compras_enviadas_F = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO="FIRMADO")
        solicitud_ordenes_compras_enviadas_R = solicitud_ordenes_compras_enviadas.filter(Q(ESTATUS_DOCUMENTO="Rechazado por Almacen") | Q(ESTATUS_DOCUMENTO="Rechazado por Compras"))
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para GERENTE DE DEPARTAMENTO DE ALMACEN') 
        ...
        solicitud_ordenes_compras_enviadas_N = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO='ENVIADO')
        solicitud_ordenes_compras_enviadas_F = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO="FIRMADO")
        solicitud_ordenes_compras_enviadas_R = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO="Rechazado por Almacen")
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para usuarios DEL DEPARTAMENTO DE ALMACEN') 
 ...
 ### ===================== ORDENES DE COMPRAS Y COTIZACIONES ===============================       
        ...
        solicitud_ordenes_compras_enviadas = solicitudOrdendeCompras.objects.all().exclude(Q(ESTATUS_DOCUMENTO="ENVIADO") | Q(ESTATUS_DOCUMENTO="Rechazado por Almacen") 	)
        #solicitud_ordenes_compras_enviadas_N = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO='ENVIADO')
        solicitud_ordenes_compras_enviadas_F = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO="FIRMADO")
        solicitud_ordenes_compras_enviadas_R = solicitud_ordenes_compras_enviadas.filter(ESTATUS_DOCUMENTO="Rechazado por Compras")
        

        # ordenes de compras  cargada por el gerente de compras
        ordenes_compra_enviadas = pdf2EnviadoGGeneral.objects.all().order_by("-fecha_modificacion")  # ordenes de compras enviados
        
        ordenes_compra_enviadas_N = ordenes_compra_enviadas.filter(ESTATUS_DOCUMENTO='ENVIADO')
        ordenes_compra_enviadas_F = ordenes_compra_enviadas.filter(Q(ESTATUS_DOCUMENTO="Firmado por GG") | Q(ESTATUS_DOCUMENTO='Firmado por Compras'))
        ordenes_compra_enviadas_R = ordenes_compra_enviadas.filter(Q(ESTATUS_DOCUMENTO="Rechazado por GG") | Q(ESTATUS_DOCUMENTO='Rechazado por Compras'))
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para GERENTE DE DEPARTAMENTO DE COMPRAS')     
        ...
        solicitud_ordenes_compras_enviadas = solicitudOrdendeCompras.objects.filter(ESTATUS_DOCUMENTO='FIRMADO').all()
        ...
        messages.success(request, f'Hola {request.user}, Contenido exclusivo para usuarios DEL DEPARTAMENTO DE COMPRAS') 
        ...
        # Contenido para otros usuarios o visitantes
        context = {'message': 'Contenido para otros usuarios'}
        messages.success(request, f'Hola {request.user}, contenido exclusivo para otros usuarios')
    
    
    return render( 
                    request,'paginas/dashboard.html',
                    context
                      )    
...
def cargarpdfOrdendeCompra(request,id):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            ...
            PDF_CAMBIAR_ESTATUS = solicitudOrdendeCompras.objects.filter(id=id).update(ESTATUS_DOCUMENTO="Generada ODC")
...
def cambiarEstatusPdf(request,id_PDF):
    ...
    return render(request, "paginas/formulario_carga_pdf.html", {"formulario":formulario})
...
def socgccambiarEstatusPdf(request,id_PDF):
    ...
    formulario = cambiarEStatuspdfolicitudordencomprasform(request.POST or None,instance=pdf)
    
    if formulario.is_valid():
        descripcion_documento=request.POST["descripcion_documento"]
    
        if user_is_in_group(request.user, 'GERENTE DE DEPARTAMENTO DE COMPRAS'):
            PDF_CAMBIAR_ESTATUS = solicitudOrdendeCompras.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Rechazado por Compras",
                                                                                           descripcion_documento = descripcion_documento)
            ...
            PDF_CAMBIAR_ESTATUS = solicitudOrdendeCompras.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Rechazado por Almacen", descripcion_documento = descripcion_documento)
...
def ordencomprasgerentecambiarEstatusPdf(request,id_PDF):
    ...
    if formulario.is_valid():
        ...
        if user_is_in_group(request.user, 'GERENCIA GENERAL'):
            PDF_CAMBIAR_ESTATUS = pdf2EnviadoGGeneral.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Rechazado por GG",
                                                                                    descripcion_documento = descripcion_documento)
            ...
            PDF_CAMBIAR_ESTATUS = pdf2EnviadoGGeneral.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Rechazado por Compras",
                                                                                        descripcion_documento = descripcion_documento)
...
def AlmacencambiarEstatusPdf(request,id_PDF):
    ...
    if formulario.is_valid():
        ...
        PDF_CAMBIAR_ESTATUS = pdfEnviadosGerente.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Rechazado por Almacen", descripcion_documento = descripcion_documento)
...
def AlmacenestatusAceptadoPdf(request,id_PDF):
    ...
    PDF_CAMBIAR_ESTATUS = pdfEnviadosGerente.objects.filter(id=id_PDF).update(ESTATUS_DOCUMENTO="Aceptado por Almacen")
...
def procesarfirma(pdf_path,firma_path,x_pdf,y_pdf):
    ...
    try:
        ...
        page.insert_image(fitz.Rect(position[0], position[1], position[0] + new_width, position[1] + new_height), 
                        filename=image_file)
        ...
        if image_data:
            ...
            if user_is_in_group(request.user, 'GERENCIA GENERAL'):
                try:
                    ...
                    pdf=pdf2EnviadoGGeneral.objects.filter(id=id_PDF).values("documento_pdf").last()
                    ...
                    with open(modified_pdf_path, 'rb') as f:
                        modified_pdf_instance = pdf2EnviadoGGeneral.objects.get(id=id_PDF)
            
                        modified_pdf_instance.ESTATUS_DOCUMENTO = "Firmado por GG"
                        modified_pdf_instance.documento_pdf.save(os.path.basename(modified_pdf_path), File(f))
            ...
            ## Gerente debe firmar el documento  requisicion    
                ...
                try:
                    ...
                    pdf=pdfEnviadosGerente.objects.filter(id=id_PDF).values("documento_pdf").last()
            ...
            #Step 12: Save the modified PDF to the database
                    with open(modified_pdf_path, 'rb') as f:
                        modified_pdf_instance = pdfEnviadosGerente.objects.get(id=id_PDF)
            ...
            ## Gerente de alamcen debe firmar el documento  solicitud de orden de compra     
                ...
                try:
                    ...
                    pdf=solicitudOrdendeCompras.objects.filter(id=id_PDF).values("documento_pdf").last()
                    
                    pdf_instance=solicitudOrdendeCompras.objects.filter(id=id_PDF).values("requisicionCancelada").last()
            ...
            #Step 12: Save the modified PDF to the database
                    with open(modified_pdf_path, 'rb') as f:
                        modified_pdf_instance = solicitudOrdendeCompras.objects.get(id=id_PDF)
